...
// ─── Hook contenu Supabase ────────────────────────────────────────────────────
const useContent = () => {
  const [content, setContent] = useState<Record<string, string>>({
    hero_title: "Maîtrisez le Prompt Engineering comme un Ingénieur.",
    hero_subtitle: "Multipliez votre productivité par 10×. Automatisez vos workflows techniques.",
...
const Programme = ({ content }: { content: Record<string, string> }) => {
  const { ref, visible } = useFadeIn();
  const modules = [
    { num: "01", key: "module1_title", subtitle: "Comprendre l'Architecture", desc: "Anatomie des LLMs, transformers, tokens, embeddings. Paramètres critiques : température, top-p. Comparatif GPT-4o, Claude 3, Gemini.", tags: ["Transformers", "Tokenisation", "Few-shot", "Fine-tuning"], accent: "text-violet-400", border: "border-violet-500/25", glow: "rgba(124,58,237,0.15)" },
    { num: "02", key: "module2_title", subtitle: "Chain-of-Thought & Anti-Hallucinations", desc: "Chain-of-Thought, Tree-of-Thought, RAG et self-consistency. Méthodes de vérification pour des sorties fiables.", tags: ["Chain-of-Thought", "RAG", "Self-Consistency", "Vérification"], accent: "text-blue-400", border: "border-blue-500/25", glow: "rgba(59,130,246,0.15)" },
    { num: "03", key: "module3_title", subtitle: "IA dans les Workflows Techniques", desc: "Agents autonomes, pipelines multi-modèles, maintenance prédictive et génération de code. Cas pratiques réels.", tags: ["LangChain", "Agents IA", "Pipelines", "API"], accent: "text-cyan-400", border: "border-cyan-500/25", glow: "rgba(6,182,212,0.15)" },
  ];
  return (
    <section ref={ref as React.RefObject<HTMLElement>} className="relative z-10 py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <div className={cn("text-center mb-16 transition-all duration-700", visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")}>
          ...
          <p className="mt-4 text-white/60 text-base font-medium max-w-xl mx-auto">
            Un parcours dense et immédiatement applicable par des profils techniques exigeants.
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-violet-500/20 p-10 md:p-14 relative overflow-hidden"
          ...
          <div className="relative z-10 text-center mb-10">
            ...
            <p className="text-white/55 text-sm font-medium">
              Les premiers inscrits bénéficient d'un <strong className="text-white/80">tarif Early Bird exclusif</strong>.
            </p>
          </div>
          ...
          {status === "success" ? (
            <div className="relative z-10 flex flex-col items-center gap-5 py-6 text-center">
              ...
              <p className="text-white/55 text-sm font-medium max-w-xs">
                Merci <strong className="text-white">{form.name}</strong> ! Vous serez notifié à{" "}
                <strong className="text-violet-300">{form.email}</strong> lors du lancement.
              </p>
              <span className="text-xs font-bold text-green-400 bg-green-500/10 border border-green-500/20 px-4 py-1.5 rounded-full">
            ...
            <form onSubmit={handleSubmit} className="relative z-10 space-y-5">
              <div>
                ...
                <input type="text" placeholder="Kouassi Williams" value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })} required
                  className="w-full bg-white/[0.06] border border-white/[0.12] rounded-xl px-5 py-4 text-white text-sm font-medium placeholder-white/25 focus:outline-none focus:border-violet-500/60 transition-all" />
              </div>
              <div>
                <label className="block text-xs font-bold text-white/55 tracking-[0.12em] uppercase mb-2">Adresse Email</label>
                <input type="email" placeholder="vous@domaine.com" value={form.email}
                  onChange={(e) => setForm({ ...form, email: e.target.value })} required
                  className="w-full bg-white/[0.06] border border-white/[0.12] rounded-xl px-5 py-4 text-white text-sm font-medium placeholder-white/25 focus:outline-none focus:border-violet-500/60 transition-all" />
              </div>
              {status === "error" && (
                <p className="text-red-400 text-xs font-medium">Une erreur est survenue. Réessayez.</p>
              )}
      ...
      <div className="flex items-center gap-4">
        {[{ label: "LinkedIn", icon: "in", href: "#" }, { label: "GitHub", icon: "gh", href: "#" }, { label: "X", icon: "𝕏", href: "#" }].map((s) => (
          <a key={s.label} href={s.href} aria-label={s.label}
            className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-xs font-bold text-white/40 hover:text-white hover:bg-white/[0.08] transition-all">
            {s.icon}
          </a>
        ))}
      </div>
...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen"
      style={{ background: "#07070f", color: "#f0f0ff", fontFamily: "'DM Sans', sans-serif" }}>
      <Background />
